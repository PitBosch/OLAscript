...
def estimate_probabilities(dataset, node_index, n_nodes):
    ...
    for episode in dataset:
        idx_w_active = np.argwhere(episode[:, node_index]==1).rehsape(-1)
        if len(idx_w_active)>0 and idx_w_active>0
            active_nodes_inprev_step=episode[idx_w_active-1,:].reshape(-1)
            credits += active_nodes_in_prev_step/np.sum(active_nodes_inprev_step)
